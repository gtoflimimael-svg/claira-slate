...
const SYSTEM_PROMPT = `You are a document page-deletion assistant. You will be given a natural-language request describing which pages to remove (e.g. "blank pages", "duplicate pages", "pages with only a logo") and a list of PDF pages (each with a 1-indexed page number and a short text snippet). Return only the page numbers that should be DELETED to satisfy the request — never suggest deleting every page.
...
function parseSuggestion(raw: string, validPages: Set<number>): { pages: number[]; reasoning: string } | null {
  ...
  // number must be one of the pages we actually sent, no duplicates, and it
  // must never cover every page (deleting an entire document is never a
  // valid suggestion — the delete route itself would also reject it, but
  // catching it here avoids showing the user a suggestion we know is dead).
  if (obj.pages.length >= validPages.size) return null;
...
export async function POST(request: NextRequest) {
  ...
  if (pages.length > MAX_PAGES) {
    return NextResponse.json({ error: `AI Delete supports up to ${MAX_PAGES} pages at once.` }, { status: 400 });
  }
  ...
  if (!suggestion) {
    return NextResponse.json(
      { error: "AI couldn't match pages to that request. Try rephrasing or mark pages manually." },
      { status: 422 }
    );
  }

  if (quota.userId) await logAIUsage(quota.userId, "delete-suggest", result.tokensUsed);
