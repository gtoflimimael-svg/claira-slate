    ...
    <div>
      ...
      <div style={{ marginTop: 24, padding: 24, border: "1px solid var(--cs-line)", borderRadius: 20, background: "var(--cs-bg)" }}>
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "baseline", gap: 12 }}>
          <div style={{ fontFamily: "var(--font-geist), Inter, sans-serif", fontSize: 56, fontWeight: 600, letterSpacing: "-.04em" }}>
            <CountUp value={316} />
          </div>
          <div style={{ fontSize: 23.62, color: "var(--cs-text-2)" }}>{t("actionsThisMonth", { count: 42 })}</div>
        </div>
        <div style={{ marginTop: 26, display: "flex", alignItems: "flex-end", gap: "clamp(8px,2vw,20px)", height: 170 }}>
          {USAGE.map((u) => (
            <div key={u.day} style={{ flex: "1 1 0", display: "flex", flexDirection: "column", alignItems: "center", gap: 10, height: "100%", justifyContent: "flex-end" }}>
              <div style={{ fontSize: 20.12, fontWeight: 600, color: "var(--cs-text-2)" }}>{u.value}</div>
              ...
              <div style={{ fontSize: 20.12, fontWeight: 500, color: "var(--cs-text-2)" }}>{u.day}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(260px,100%),1fr))", gap: 14 }}>
        <div style={{ padding: 22, border: "1px solid var(--cs-line)", borderRadius: 20, background: "var(--cs-bg)" }}>
          <div style={{ fontSize: 22.75, fontWeight: 600 }}>{t("byFeature")}</div>
          <div style={{ marginTop: 18, display: "flex", flexDirection: "column", gap: 16 }}>
            {BY_FEATURE.map((f) => (
              <div key={f.key}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 22.75 }}>
                  <span>{t(f.key)}</span>
        ...
        <div style={{ padding: 22, border: "1px solid var(--cs-line)", borderRadius: 20, background: "var(--cs-bg)", display: "flex", flexDirection: "column" }}>
          <div style={{ fontSize: 22.75, fontWeight: 600 }}>{t("limits")}</div>
          <div style={{ marginTop: 18, display: "flex", flexDirection: "column", gap: 12, fontSize: 23.62, color: "var(--cs-text-2)" }}>
          ...
          <Link href="/app/billing" style={{ marginTop: "auto", paddingTop: 20, fontSize: 22.75, fontWeight: 500, cursor: "pointer" }}>
            {t("managePlan")} &rarr;
          </Link>
        </div>
      </div>
