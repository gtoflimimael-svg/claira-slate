    ...
    <div>
      ...
      <div style={{ marginTop: 24, display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(min(220px,100%),1fr))", gap: 12 }}>
        ...
        >
          ...
          <div style={{ fontSize: 23.62, fontWeight: 500 }}>{t("dropHere")}</div>
          <div style={{ fontSize: 21, color: "var(--cs-text-2)" }}>{t("upTo2gb")}</div>
        </Link>
        ...
        {HISTORY.map((h) => {
          ...
          return (
            <div key={h.name} style={{ display: "flex", flexDirection: "column", minHeight: 184, padding: 18, border: "1px solid var(--cs-line)", borderRadius: "var(--cs-r)", background: "var(--cs-bg)" }}>
              <div style={{ width: 32, height: 40, borderRadius: 5, border: "1px solid var(--cs-line)", background: "var(--cs-bg-2)", display: "grid", placeItems: "center", fontSize: 14.88, fontWeight: 600, color: "var(--cs-bad)" }}>
                PDF
              </div>
              <div style={{ marginTop: 14, fontSize: 23.62, fontWeight: 500, lineHeight: 1.35, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{h.name}</div>
              <div style={{ marginTop: 6, fontSize: 21, color: "var(--cs-text-2)" }}>
                {h.pages} · {h.size}
              </div>
              <div style={{ marginTop: "auto", paddingTop: 14, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                ...
                <span style={{ fontSize: 20.12, color: "var(--cs-text-2)" }}>{h.when}</span>
              </div>
            </div>
          );
        })}
