...
const STEPS = [
  {
    ...
    desc: "We read the whole thing once, so every question after that is instant.",
    icon: (
      <>
  ...
  },
  {
    n: "02",
    title: "Ask anything",
    desc: "Questions are answered strictly from the document's own content.",
    icon: <path d="M21 14a3 3 0 0 1-3 3H8l-5 4V6a3 3 0 0 1 3-3h12a3 3 0 0 1 3 3z"></path>,
  },
  {
    n: "03",
    title: "Check the source",
    desc: "Every answer links back to the page or section it came from.",
    icon: (
      <>
    ...
    <div>
      <section style={{ maxWidth: 1100, margin: "0 auto", padding: "clamp(48px,6vw,88px) 24px 0" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, fontWeight: 500, color: "var(--cs-text-2)" }}>
          ...
          <span style={{ color: "var(--cs-line)" }}>/</span>
          <span style={{ color: "var(--cs-text)" }}>Chat</span>
        </div>
        <div style={{ marginTop: "clamp(28px,3.5vw,44px)", display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 18 }}>
          ...
          <p style={{ margin: 0, maxWidth: 520, fontSize: "clamp(15px,1.4vw,18px)", lineHeight: 1.6, color: "var(--cs-text-2)", textWrap: "pretty" }}>
            Upload a document and chat with it directly — every answer cites the section or page it came from.
          </p>
        </div>
      </section>
