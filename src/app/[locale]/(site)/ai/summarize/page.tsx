...
import { SummarizeTool } from "@/components/ai/summarize-tool";

export const metadata = {
  title: "Summarize a PDF — Claira Slate",
  description: "Drop in a report, contract or deck and get the key points — with the page each one came from.",
};
...
const STEPS = [
  {
    ...
    desc: "Text or scanned — OCR runs automatically when needed.",
    icon: (
      <>
  ...
  {
    n: "02",
    title: "Pick a summary length",
    desc: "Four bullets, one page, or a section-by-section brief.",
    icon: <path d="M13.5 2.5 4 14h6.5L9.5 21.5 20 10h-7z"></path>,
  },
  {
    n: "03",
    title: "Copy, export or ask more",
    desc: "Keep the summary, or start a chat with the same file.",
    icon: (
      <>
...
const OTHER_AI = [
  { title: "Ask anything", desc: "Chat with your document.", href: "/ai/chat", icon: <path d="M21 14a3 3 0 0 1-3 3H8l-5 4V6a3 3 0 0 1 3-3h12a3 3 0 0 1 3 3z"></path> },
  { title: "Translate", desc: "50+ languages, layout preserved.", href: "/ai/translate", icon: <path d="M4 6h9M8.5 6c0 5-2 8-5.5 10M6 11c1.5 3 4 4.5 7 5M13 20l4-10 4 10M14.6 17h4.8"></path> },
  { title: "OCR", desc: "Make scanned PDFs searchable.", href: "/ai/ocr", icon: <path d="M4 8V5h3M20 8V5h-3M4 16v3h3M20 16v3h-3M8 12h8"></path> },
  { title: "Merge PDF", desc: "Combine files into one.", href: "/tools/merge", icon: <path d="M3 3h10v10H3zM11 11h10v10H11z"></path> },
];
    ...
    <div>
      <section style={{ maxWidth: 1100, margin: "0 auto", padding: "clamp(48px,6vw,88px) 24px 0" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, fontWeight: 500, color: "var(--cs-text-2)" }}>
          ...
          <Link href="/ai" className="hover-text" style={{ cursor: "pointer", color: "var(--cs-text-2)" }}>AI</Link>
          <span style={{ color: "var(--cs-line)" }}>/</span>
          <span style={{ color: "var(--cs-text)" }}>Summarize</span>
        </div>
        <div style={{ marginTop: "clamp(28px,3.5vw,44px)", display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 18 }}>
          <span style={{ padding: "5px 11px", borderRadius: 99, background: "var(--cs-accent)", color: "#fff", fontSize: 11.5, fontWeight: 600 }}>AI tool</span>
          ...
          <p style={{ margin: 0, maxWidth: 520, fontSize: "clamp(15px,1.4vw,18px)", lineHeight: 1.6, color: "var(--cs-text-2)", textWrap: "pretty" }}>
            Drop in a report, contract or deck and get the key points — with the page each one came from. Free for two documents a day.
          </p>
        </div>
      </section>
      ...
      <section style={{ maxWidth: 1100, margin: "0 auto", padding: "clamp(56px,7vw,96px) 24px clamp(56px,7vw,96px)" }}>
        ...
        <div style={{ marginTop: 26, display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(min(232px,100%),1fr))", gap: 12 }}>
          {OTHER_AI.map((t, i) => (
            <Reveal key={t.title} variant="scale" index={i}>
              <Link
                data-lift
                data-card
                href={t.href}
              ...
              >
                ...
                <div data-shine style={{ marginTop: 14, fontSize: 14.5, fontWeight: 600, letterSpacing: "-.015em" }}>{t.title}</div>
                <div style={{ marginTop: 5, fontSize: 13, lineHeight: 1.45, color: "var(--cs-text-2)" }}>{t.desc}</div>
              </Link>
            </Reveal>
          ))}
        </div>
