import { Link } from "@/i18n/navigation";
import { Reveal } from "@/components/reveal";
import { OcrTool } from "@/components/ai/ocr-tool";

export const metadata = {
  title: "OCR a scanned PDF — Claira Slate",
  description: "Turn a scanned PDF into a searchable, selectable document.",
};

const STEPS = [
  {
    n: "01",
    title: "Upload the scan",
    desc: "Each page is rendered and read individually.",
    icon: (
      <>
        <path d="M4 14.9A7 7 0 1 1 15.7 8h1.8a4.5 4.5 0 0 1 2.5 8.24"></path>
        <path d="M12 21v-8"></path>
        <path d="M8 17l4-4 4 4"></path>
      </>
    ),
  },
  {
    n: "02",
    title: "Text is extracted and cleaned",
    desc: "OCR runs on every page, then AI fixes obvious errors and spacing.",
    icon: <path d="M4 8V5h3M20 8V5h-3M4 16v3h3M20 16v3h-3M8 12h8"></path>,
  },
  {
    n: "03",
    title: "Download a searchable PDF",
    desc: "Looks the same, but every word is now selectable and searchable.",
    icon: (
      <>
        <path d="M12 3v12"></path>
        <path d="M7.5 10.5 12 15l4.5-4.5"></path>
        <path d="M4 20h16"></path>
      </>
    ),
  },
];

const OTHER_AI = [
  { title: "Summarize", desc: "4-6 bullet points, with page refs.", href: "/ai/summarize", icon: <path d="M13.5 2.5 4 14h6.5L9.5 21.5 20 10h-7z"></path> },
  { title: "Ask anything", desc: "Chat with your document.", href: "/ai/chat", icon: <path d="M21 14a3 3 0 0 1-3 3H8l-5 4V6a3 3 0 0 1 3-3h12a3 3 0 0 1 3 3z"></path> },
  { title: "Translate", desc: "50+ languages, layout preserved.", href: "/ai/translate", icon: <path d="M4 6h9M8.5 6c0 5-2 8-5.5 10M6 11c1.5 3 4 4.5 7 5M13 20l4-10 4 10M14.6 17h4.8"></path> },
  { title: "Merge PDF", desc: "Combine files into one.", href: "/tools/merge", icon: <path d="M3 3h10v10H3zM11 11h10v10H11z"></path> },
];

export default function OcrPage() {
  return (
    <div>
      <section style={{ maxWidth: 1100, margin: "0 auto", padding: "clamp(48px,6vw,88px) 24px 0" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, fontWeight: 500, color: "var(--cs-text-2)" }}>
          <Link href="/" className="hover-text" style={{ cursor: "pointer", color: "var(--cs-text-2)" }}>Home</Link>
          <span style={{ color: "var(--cs-line)" }}>/</span>
          <Link href="/ai" className="hover-text" style={{ cursor: "pointer", color: "var(--cs-text-2)" }}>AI</Link>
          <span style={{ color: "var(--cs-line)" }}>/</span>
          <span style={{ color: "var(--cs-text)" }}>OCR</span>
        </div>
        <div style={{ marginTop: "clamp(28px,3.5vw,44px)", display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 18 }}>
          <span style={{ padding: "5px 11px", borderRadius: 99, background: "var(--cs-accent)", color: "#fff", fontSize: 11.5, fontWeight: 600 }}>AI tool</span>
          <h1 style={{ margin: 0, fontFamily: "var(--font-geist), Inter, sans-serif", fontWeight: 600, fontSize: "clamp(34px,5vw,56px)", lineHeight: 1.04, letterSpacing: "-.04em" }}>
            Make a scan searchable
          </h1>
          <p style={{ margin: 0, maxWidth: 520, fontSize: "clamp(15px,1.4vw,18px)", lineHeight: 1.6, color: "var(--cs-text-2)", textWrap: "pretty" }}>
            Drop in a scanned PDF and get back the same document with selectable, searchable text.
          </p>
        </div>
      </section>

      <section style={{ maxWidth: 1100, margin: "0 auto", padding: "clamp(36px,5vw,60px) 24px 0" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(320px,100%),1fr))", gap: 16, alignItems: "start" }}>
          <OcrTool />
        </div>
      </section>

      <section style={{ maxWidth: 1100, margin: "0 auto", padding: "clamp(56px,7vw,96px) 24px 0" }}>
        <Reveal as="h2" style={{ margin: 0, fontFamily: "var(--font-geist), Inter, sans-serif", fontWeight: 600, fontSize: "clamp(26px,3.4vw,38px)", lineHeight: 1.08, letterSpacing: "-.035em" }}>
          How OCR works
        </Reveal>
        <div data-steps style={{ marginTop: "clamp(28px,4vw,44px)", display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 0 }}>
          {STEPS.map((s, i) => (
            <div
              key={s.n}
              style={{
                padding: i === 0 ? "26px 26px 26px 0" : "26px",
                borderTop: i === 0 ? "1.5px solid var(--cs-text)" : "1.5px solid var(--cs-line)",
                borderLeft: i === 0 ? undefined : "1px solid var(--cs-line)",
              }}
            >
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ flex: "none", width: 56, height: 56, borderRadius: "50%", background: "var(--cs-grad-135)", display: "grid", placeItems: "center", color: "#fff" }}>
                  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round">
                    {s.icon}
                  </svg>
                </div>
                <div style={{ fontFamily: "var(--font-geist), Inter, sans-serif", fontSize: 13, fontWeight: 600, letterSpacing: ".02em", color: "var(--cs-accent)" }}>{s.n}</div>
              </div>
              <div style={{ marginTop: 18, fontFamily: "var(--font-geist), Inter, sans-serif", fontSize: "clamp(17px,1.8vw,20px)", fontWeight: 600, letterSpacing: "-.025em" }}>{s.title}</div>
              <div style={{ marginTop: 8, fontSize: 14, lineHeight: 1.55, color: "var(--cs-text-2)", maxWidth: 260 }}>{s.desc}</div>
            </div>
          ))}
        </div>
      </section>

      <section style={{ maxWidth: 1100, margin: "0 auto", padding: "clamp(56px,7vw,96px) 24px clamp(56px,7vw,96px)" }}>
        <Reveal as="h2" style={{ margin: 0, fontFamily: "var(--font-geist), Inter, sans-serif", fontWeight: 600, fontSize: "clamp(26px,3.4vw,38px)", lineHeight: 1.08, letterSpacing: "-.035em" }}>
          Other AI tools
        </Reveal>
        <div style={{ marginTop: 26, display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(min(232px,100%),1fr))", gap: 12 }}>
          {OTHER_AI.map((t, i) => (
            <Reveal key={t.title} variant="scale" index={i}>
              <Link
                data-lift
                data-card
                href={t.href}
                style={{ display: "block", padding: 18, border: "1px solid var(--cs-line)", borderRadius: "var(--cs-r)", background: "var(--cs-card)", color: "var(--cs-text)", cursor: "pointer" }}
              >
                <div style={{ width: 34, height: 34, borderRadius: 8, background: "var(--cs-accent-soft)", border: "1px solid var(--cs-accent-line)", display: "grid", placeItems: "center", color: "var(--cs-accent)" }}>
                  <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round">
                    {t.icon}
                  </svg>
                </div>
                <div data-shine style={{ marginTop: 14, fontSize: 14.5, fontWeight: 600, letterSpacing: "-.015em" }}>{t.title}</div>
                <div style={{ marginTop: 5, fontSize: 13, lineHeight: 1.45, color: "var(--cs-text-2)" }}>{t.desc}</div>
              </Link>
            </Reveal>
          ))}
        </div>
      </section>
    </div>
  );
}
