    ...
    <section id="ai" style={{ position: "relative", background: "var(--cs-ink)", color: "#fff", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", maxWidth: 1200, margin: "0 auto", padding: "clamp(72px,9vw,120px) 24px" }}>
        <Reveal
          ...
          variant="fade"
          data-spinborder
          style={{ display: "inline-block", padding: "6px 13px", borderRadius: 99, background: "var(--cs-accent)", color: "#fff", fontSize: 18, fontWeight: 600, letterSpacing: ".01em" }}
        >
          {th("badge")}
        </Reveal>
        <Reveal as="h2" style={{ margin: "22px 0 0", maxWidth: 760, fontFamily: "var(--font-geist), Inter, sans-serif", fontWeight: 600, fontSize: "clamp(48px,7.5vw,84px)", lineHeight: 1.04, letterSpacing: "-.038em", color: "#fff" }}>
          {th("headline")}
        </Reveal>
        <p style={{ margin: "18px 0 0", maxWidth: 520, fontSize: 25.5, lineHeight: 1.6, color: "rgba(255,255,255,.62)" }}>
          {th("subhead")}
        </p>
        ...
        <div style={{ marginTop: "clamp(40px,5vw,64px)", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(238px,1fr))", gap: 14 }}>
          {FEATURES.map((f, i) => (
            ...
            >
              ...
              <div style={{ marginTop: 18, fontFamily: "var(--font-geist), Inter, sans-serif", fontSize: 25.5, fontWeight: 600, letterSpacing: "-.02em" }}>{t(`${f.key}.name`)}</div>
              <div style={{ marginTop: 7, fontSize: 21, lineHeight: 1.5, color: "rgba(255,255,255,.58)" }}>{t(`${f.key}.desc`)}</div>
            </Reveal>
          ))}
        </div>
        <div style={{ marginTop: 38 }}>
          <Link
            ...
            data-arrow
            href="/ai"
            style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "13px 22px", borderRadius: 10, background: "var(--cs-grad)", color: "#fff", fontSize: 22.5, fontWeight: 500, cursor: "pointer" }}
          >
