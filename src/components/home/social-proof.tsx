    ...
    <section id="trust" style={{ borderTop: "1px solid var(--cs-line)", background: "var(--cs-bg)" }}>
      <div style={{ padding: "clamp(64px,8vw,104px) 24px" }}>
          ...
          <div>
            <div
              style={{
                ...
                fontSize: "clamp(49px,5.95vw,70px)",
                fontWeight: 600,
                letterSpacing: "-.04em",
            ...
            <div style={{ marginTop: 4, fontSize: 23.62, fontWeight: 500, color: "var(--cs-text-2)" }}>{t("docsCaption")}</div>
          </div>
        </div>
        ...
        <div style={{ marginTop: "clamp(36px,5vw,52px)", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(272px,1fr))", gap: 14 }}>
          {TESTIMONIALS.map((item, i) => (
            <Reveal key={item.key} index={i} data-lift style={{ padding: 26, border: "1px solid var(--cs-line)", borderRadius: "var(--cs-r)", background: "var(--cs-card)" }}>
              <p style={{ margin: 0, fontSize: 27.12, lineHeight: 1.6, letterSpacing: "-.012em" }}>&ldquo;{t(`${item.key}Quote`)}&rdquo;</p>
              <div style={{ marginTop: 22, display: "flex", alignItems: "center", gap: 11 }}>
                <div style={{ width: 34, height: 34, borderRadius: 8, background: "var(--cs-accent-soft)", border: "1px solid var(--cs-accent-line)", display: "grid", placeItems: "center", fontSize: 21.88, fontWeight: 600, color: "var(--cs-accent)" }}>
                  {item.initials}
                </div>
                <div>
                  <div style={{ fontSize: 23.62, fontWeight: 600 }}>{item.name}</div>
                  <div style={{ fontSize: 21.88, color: "var(--cs-text-2)" }}>{t(`${item.key}Role`)}</div>
                </div>
              </div>
            </Reveal>
          ))}
        ...
        <div style={{ marginTop: 34, display: "flex", flexWrap: "wrap", gap: 10 }}>
          {BADGES.map((b) => (
            <div key={b.key} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 15px", border: "1px solid var(--cs-line)", borderRadius: 99, fontSize: 22.75, fontWeight: 500, color: "var(--cs-text-2)" }}>
