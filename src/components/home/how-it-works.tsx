...
const STEPS: { icon: React.ReactNode; num: string; title: string; desc: string; variant: "left" | undefined | "right" }[] = [
  {
    icon: (
      <>
    ...
    desc: "Drag it in from your desktop, Drive or Dropbox. Up to 2 GB.",
    variant: "left",
  },
  {
    icon: <path d="M13.5 2.5 4 14h6.5L9.5 21.5 20 10h-7z"></path>,
    num: "02",
    title: "Choose a tool or ask AI",
    desc: "Pick from 26 tools — or just type what you want done.",
    variant: undefined,
  },
  {
    icon: (
    ...
    desc: "No watermarks, no email gate. Your file is deleted after an hour.",
    variant: "right",
  },
];
        ...
        <div data-steps style={{ marginTop: "clamp(36px,5vw,56px)", display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 0 }}>
          {STEPS.map((s, i) => (
            ...
            >
              ...
              <div style={{ marginTop: 20, fontFamily: "var(--font-geist), Inter, sans-serif", fontSize: "clamp(19px,2vw,23px)", fontWeight: 600, letterSpacing: "-.025em" }}>{s.title}</div>
              <div style={{ marginTop: 9, fontSize: 14.5, lineHeight: 1.55, color: "var(--cs-text-2)", maxWidth: 280 }}>{s.desc}</div>
            </Reveal>
          ))}
        </div>
      </div>
